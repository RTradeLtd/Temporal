...
func (api *API) ConfirmPayment(c *gin.Context) {
	...
	qm, err := queue.New(queue.PaymentConfirmationQueue, api.cfg.RabbitMQ.URL, true)
	if err != nil {
...
func (api *API) CreateDashPayment(c *gin.Context) {
	...
	qm, err := queue.New(queue.DashPaymentConfirmationQueue, api.cfg.RabbitMQ.URL, true)
	if err != nil {
		api.LogError(err, eh.QueueInitializationError)(c, http.StatusBadRequest)
		return
	}
