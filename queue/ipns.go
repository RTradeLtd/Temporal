		...
		case d := <-msgs:
			...
			go func(d amqp.Delivery) {
				...
				if _, err = ipnsManager.FindByIPNSHash(id.Pretty()); err != nil {
					_, err = ipnsManager.CreateEntry(id.Pretty(), ie.CID, ie.Key, ie.NetworkName, ie.UserName, ie.LifeTime, ie.TTL)
				} else {
					_, err = ipnsManager.UpdateIPNSEntry(id.Pretty(), ie.CID, ie.NetworkName, ie.UserName, ie.LifeTime, ie.TTL)
				}
				if err != nil {
